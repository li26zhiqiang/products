        ...
        <el-page-header @back="onBack">
            ...
            <template #content>
                <span class="text-large font-600 mr-3">{{
                    this.$route.query.id ? '修改商品分类' : '添加商品分类'
                }}</span>
            </template>
        </el-page-header>
        ...
        <el-card shadow="never">
            <el-form :model="productCate" :rules="rules" ref="productCateFrom" label-width="150px">
                ...
                <el-form-item label="分类图标：">
                    <single-upload v-model:value="productCate.icon"></single-upload>
                </el-form-item>
                <el-form-item
                    v-for="(filterProductAttr, index) in filterProductAttrList"
                    :label="filterLabelFilter_filter(index)"
                    :key="filterProductAttr.key"
                >
                    <el-cascader clearable v-model="filterProductAttr.value" :options="filterAttrs"> </el-cascader>
                    <el-button
                        type="danger"
...
export default {
    ...
    methods: {
        ...
        filterLabelFilter_filter(index) {
            if (index === 0) {
                return '筛选属性：';
            } else {
                return '';
